...
export default function MessagesPage() {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const others = Array.isArray(list)
          ? list.filter((u: User) => u.id !== me?.id)
          : []
        setUsers(others)
        setError('')  // clear any previous error on success
      }
      // silently ignore non-ok on poll retries
    } catch {
      // only show error if we have no users yet (first load fail)
      setUsers(prev => {
        if (prev.length === 0) setError('Khong the tai danh sach nguoi dung')
        return prev
      })
    } finally {
  ...
  useEffect(() => {
    ...
    fetchUsers()
  }, [fetchUsers])  // stable ref - only runs once on mount
  ...
  const fetchMessages = useCallback(async (userId: number) => {
    ...
    if (!token) return
    try {
      const res = await fetch(`${API_BASE}/api/v1/messages/${userId}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      if (res.ok) {
        const data = await res.json()
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(err.error || 'Khong the gui tin nhan')
      }
    } catch {
