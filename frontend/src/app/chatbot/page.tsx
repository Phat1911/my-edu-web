...
export default function ChatbotPage() {
  ...
  const [sessionId] = useState(() => {
    if (typeof window !== 'undefined') {
      const s = localStorage.getItem('buddy_sid'); if (s) return s
      const n = genId(); localStorage.setItem('buddy_sid', n); return n
    }
    return genId()
  })
  ...
  useEffect(() => {
    fetch(apiUrl + '/chat/' + sessionId).then(r => r.json()).then(d => {
      if (d.data && d.data.length > 0) setMessages(d.data)
      else setMessages([{id:0, session_id:sessionId, role:'assistant' as const,
        content:'Xin chao! Minh la Buddy AI - nguoi ban dong hanh tam ly 24/7 cua ban. Hay chia se bat cu dieu gi ban muon nhe!',
        created_at: new Date().toISOString()}])
    }).catch(() => setMessages([{id:0, session_id:sessionId, role:'assistant' as const,
  ...
  const handleSend = async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch(apiUrl + '/chat', {method:'POST', headers:{'Content-Type':'application/json'},
        body:JSON.stringify({session_id:sessionId, message:sentInput})})
      if (!res.ok) throw new Error('HTTP ' + res.status)
      const data = await res.json()
      const reply = data.response || 'Co loi xay ra. Vui long thu lai.'
      setMessages(p => [...p, {id:Date.now()+1, session_id:sessionId, role:'assistant' as const, content:reply, created_at:new Date().toISOString()}])
    } catch {
      setMessages(p => [...p, {id:Date.now()+1, session_id:sessionId, role:'assistant' as const, content:'Xin loi, co loi xay ra. Vui long thu lai!', created_at:new Date().toISOString()}])
